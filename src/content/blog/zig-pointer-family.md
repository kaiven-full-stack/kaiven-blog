...
description: C 的 T* 一个写法承担了全部处境：单项、数组、可空、终点，全靠约定区分。Zig 把这些含混拆成一整个类型家族，长度、哨兵、可空性都写进类型。类型与尺寸输出取自 Zig 0.16.0。
pubDate: 2026-09-04
category: zig
tags: [Zig, 编程语言]
---

地址负责指明去处，却说明不了去处之内有什么。只有一个地址，你不知道它指向一个对象，还是一整段数组的开头；不知道这段内存有多长，也不知道末尾有没有终止记号。要继续往前走，就只能靠猜。

C 里的指针正是这样一个光秃秃的地址。`T *` 可以指向一个 `T`，可以指向数组首项，可以为空，也可以一路做算术走到别处。长度写在另一个参数里，终点藏在一个零后面，所有权与可写性散落在注释和约定中。一个星号承担了太多含义，最后几乎什么都没说清。
...
星号旁边长出了方括号、问号、数字和冒号。初见像一族面目相近的生字，叫人疑心这是否只是语法洁癖。其实它们都在回答同一个问题：**拿到这个地址时，编译器究竟还握有多少信息？**

上一篇写整数溢出，谈的是一个值走到类型边界以后该去哪里；这一篇换一个方向，谈地址从哪里起、可以走多远，以及边界信息丢失以后，谁还能替你作保。代码全部在 Zig 0.16.0 上验证过。
...
`*u16` 的意思是「一个地址，那里恰好有一个 `u16`」，仅此而已。所以它用 `p.*` 解引用，不支持 `p[1]`，也不支持 `p + 1`。若硬写：
...
Zig 会做地址加法。问题在于 `*u16` 的类型里没有「这里是一串东西」这条信息。让它往后走一步，等于断定隔壁也归同一户，编译器不肯替你猜。
...
一个管指针本身能不能换，一个管目标能不能动。写法挨得很近，含义并不是一回事。

## `*[N]T`：连长度也写进类型

若指向的不是一件东西，而是一整个数组，地址可以带上数组的完整类型：
...
`*[4]u8` 仍是单项指针，只不过它指向的那「一项」恰好是 `[4]u8`。长度 4 不在运行时另存一份，它就写在类型里。于是编译器知道 `p[2]` 合法，也知道 `p[4]` 已过界。

这类数组指针是 Zig 指针家族里的枢纽，它手里的信息最齐：地址有了，元素类型有了，长度也是编译期常量。需要时，它可以把部分信息交出去：
...
反过来却没有这样从容。一个 `[*]u8` 本来就没带长度，不能凭空变回 `*[4]u8`；已经丢失的信息，类型系统不会替你补写。
...
`[]const u32` 的意思是「一段连续的 `u32`，起点和长度都在」。语言参考说得很直白：slice is a pointer and a length——切片是一枚指针，加一个长度。
...
「指针加长度」是语言语义；具体两个字在内存里谁先谁后、有没有 padding，却不能拿去当 ABI 用。可靠的入口只有 `.ptr` 和 `.len`。能在当前机器上观察到布局，不等于语言替所有目标、所有版本签了字。
...
`slice[100]` 有长度可查，Debug 下能报告越界；`raw[100]` 连「100 是否太大」这个问题都无从提出。实测即便在 Debug，`[*]T` 的索引也不会自动使用原切片的长度——那份长度已经留在身后了。若地址最终落到对象之外，依然是非法行为，只是类型已经没有足够信息替你布置检查。

同理，不要把推进切片写成修改 `slice.ptr`。指针向前走了，`.len` 却仍停在原地，起点与边界就对不上了。正确做法是重新切片：

```zig
slice = slice[1..];
```

新起点和新长度一同生成，两份信息仍然一致。
...
ReleaseFast 会移除检查，错误承诺便可能继续流入后面的代码。哨兵写进类型，事实不会凭空生成；从无保证的内存收紧到有保证的类型，总得有人核验。
...
它不肯因为最后一个字节「也许碰巧是零」就替你放行。类型要的是证据，不是愿望。
...
null 在 Zig 里是类型明确允许的一种缺席，与地址数值是否恰好为零无关。要用，先写问号。

## 对齐与 volatile：地址还能附带别的约束
...
`const`、`align`、`volatile`、`allowzero` 看似又给类型添了许多笔画，其实仍在做同一件事：把原本散在注释里的前提，尽量收进编译器能读懂的地方。
...
它之所以如此宽松，是因为翻译 C 头文件时，单看一个 `T *` 根本无法知道那是单个对象、数组首项还是 nullable pointer。`[*c]T` 是为了把这份原始含混原样接住。

语言参考对此几乎没有留情：应尽可能避免这种类型，合理用途是自动翻译 C 代码。原生 Zig API 若知道参数是单项，就写 `*T`；知道是一段数据，就写 `[]T`；可能为空，就写 `?*T`；以零结尾，就写 `[*:0]T`。信息明明在手，却主动退回 `[*c]T`，等于把好不容易分开的承诺重新揉成一团。
...
信息可以被你亲手丢掉。从切片取 `.ptr`，边界检查便失去长度；用 `@ptrCast` 改换子类型，后续读写可能踏进不可检测的非法行为；ReleaseFast 又会移除许多安全检查。类型细，不代表语言自动安全。Zig 只是让放弃了哪一份保护尽量可见，不会锁住所有后门。
...
与 C 交界时，含混会回来。`[*c]T` 的存在就是承认：外部 ABI 未必提供 Zig 想要的证据。绑定层要检查 null、补上长度、核验 sentinel，再尽快把 C pointer 收紧成原生类型。这些胶水之所以存在，是因为两个世界对地址的承诺不同，边界处必须有人翻译。

---

到这里，那枚孤零零的地址终于补齐了凭据。`*T` 说门后只有一件东西；`*[N]T` 把长度写进类型；`[]T` 把边界随身带着；`[*]T` 只认起点；哨兵指针在终点立记号；optional pointer 承认地址也可能缺席；`[*c]T` 把 C 没说清的话原样封存。

上一篇写整数的边界，这一篇写内存的边界。两者其实问的是同一件事：当程序向前走一步，凭什么相信这一步仍在合法范围之内？整数靠位宽作答，切片靠长度作答，哨兵靠终点作答；没有答案的地方，Zig 宁愿换一种类型，直说自己不知道。

知道多少，类型里就写多少；一无所知，也不要冒充胸有成竹。下一篇讲内存布局——地址有了凭据之后，字段在结构体里怎么落座。
