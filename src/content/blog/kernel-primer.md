...
推荐顺序：0 → 1 → 2 是一条主线（地址 → 翻译 → 写入），3 → 4 是物资线（仓库 → 柜台），5 收束地址线，6 → 7 讲释放（先落盘缓存，再最后保险）。赶时间的话，0 → 1 → 2 读完你就已经跨过了这个系列最陡的坡；3 到 7 每篇开头都有独立的问题引入，单读也成立。番外不在主线里，讲的是主线之前和主线之外的事，任何时候单读都行。
