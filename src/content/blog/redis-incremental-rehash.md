...
description: 百万键的字典要扩容，一次性搬家会锁住整个大厅。本文把一次扩容冻结在半途，拆开两张表的分工、读驱动的顺手搬家、serverCron 的时间预算、SCAN 的倒序游标与子进程期间的全面暂停，看清 Redis 如何把一次大迁移摊进日常流量。文中实验在 Redis 7.4.11 上复核。
pubDate: 2026-09-10
category: redis
tags: [Redis, 数据库]
---
...
```text
旧表 1,048,576 桶   还住着 1,012,381 个键
新表 2,097,152 桶   只搬进来    47,619 个键

七分钟过去，两个数字一动不动。
这期间 GET、SET、SCAN、DBSIZE，一切照常。
```

搬家进行到百分之四，就停在了那里。没有后台线程接手，没有错误日志，也没有任何客户端感觉到异样。

这不是故障现场，是我在隔离容器里刻意冻住的一次扩容。Redis 的字典在容量不够时会把桶数翻倍，但它并不一次搬完：旧表和新表并存，键分批迁移，每次读写顺手搬一点，空闲时后台预算再补一点。**大迁移被摊进了日常流量里，摊不完，也不影响营业。**
...
百万键的场景下，最后一次扩容是从 1,048,576 桶到 2,097,152 桶。一次搬完意味着主线程要连续挪动上百万个条目、重新分配一整张 16MB 的桶数组——上一篇已经算过，这正是事件循环最怕的那种长命令。渐进式 rehash 就是为了不付这笔一次性的账。
...
新表多 5：新键本身进了新表，插入路径顺手搬走旧表一桶里的 4 个键。旧表的计数从此只减不增——**搬家期间，所有新住客都直接搬进新楼，没有人再住旧楼。**
...
流量会停，搬家不能无限期挂着。兜底的是事件循环篇介绍过的 `serverCron`：默认每 100 毫秒运行一次，其中有一段专门推进迁移——但有时间预算。
...
键被删掉，表也要跟着缩小，否则空桶占内存。缩容阈值是对称的镜像：**元素少于桶数的 1/8（`HASHTABLE_MIN_FILL` 为 8），就缩到不小于元素数的最小 2 的幂。** 与扩容一样由插入删除路径的 `dictShrinkIfNeeded()` 和 cron 的轮询共同检查。
...
这里有个工程上值得留意的推论：**负载在 1/8 阈值附近震荡的负载模式，会反复触发缩容与扩容。** 每次 resize 都要分配新桶数组、迁移全部条目，虽然渐进式把成本摊薄了，但两张表并存的窗口里 SCAN 要扫更多桶、`dictShrink` 也会碰内存分配器。键数在阈值附近周期性波动的场景（比如每天批量导入再清空的作业），实际会不断搬家。这类场景可以在低峰期观察 `DEBUG HTSTATS` 的表尺寸是否反复跳动。

顺带一提，缩容时两张表里**新表是小的那张**，SCAN 的「小表定位、大表连扫」逻辑通过比较尺寸自动适应方向，不关心谁新谁旧。
...
- `CONFIG SET activerehashing no`：只在隔离实验里冻住现场用，生产环境不要关——cron 那条腿没了，冷键的桶就只能等读写了。
...
**渐进式换来的不是「没有成本」，是「成本不再有尖峰」。**

## 账单，摊开以后再算

**扩容在负载因子 1 时触发，翻倍分配。** 1,048,576 桶撑到第 1,048,577 个键，就启动向 2,097,152 桶的迁移。触发瞬间只分配不搬运，两张表立刻并存。

**搬家以桶为单位，读写顺手执行。** 一次 GET 或 SET 最多搬一个非空桶，桶里串着的键一次带走。十万次随机 GET 搬走约 17–19 万键，且新键只进新表，旧表只出不进。

**serverCron 提供每秒 10 毫秒的兜底预算。** 没有流量的冷表靠 cron 排空，速度约每秒 1.7 万条；`activerehashing` 是这个开关，默认开。

**SCAN 靠倒序二进制游标保证不漏。** 0→6→1→3→7 的顺序从低位向高位扩张，扩容分裂出的新桶永远落在未走的路径上。百万键双表状态下全量 SCAN 与 DBSIZE 精确一致。

**缩容在负载 1/8 时触发，缩到阈值边缘。** 209 万桶删到只剩 25 万键时缩回 26 万桶，负载立即回到 98.6%。阈值附近的负载震荡会反复触发搬家，是容量规划时值得避开的状态。

**fork 子进程期间，迁移全面暂停。** AVOID 策略不仅冻结新 resize，连读驱动的搬运都停——十万次 GET 搬动 0 键。这是给写时复制的让路：成片改写内存的搬家，正是 COW 最怕的访存模式。

---

```text
t0        第 1,048,577 个键落进大厅，新楼破土。
t0+1ms    旧楼第一桶住客搬进新楼，大厅照常开门。
t0+0.5s   十万次 GET 经过，十八万键换了楼。
t0+15s    流量退去，cron 一毫秒一毫秒地扫尾。
t0+16s    旧楼拆除，只剩一张 2,097,152 桶的表。
```

从第一个键跨过阈值到最后一个桶搬空，没有任何一个瞬间值得写进慢日志。所谓渐进式，不是把搬家变快了——一百万个键还是要一百万次挪动——而是把「一次让所有人等待的大迁移」，变成「一万次谁也没察觉的小顺手」。

大厅里的人只看到门一直开着。物业的账本上，搬家从来没停过。
