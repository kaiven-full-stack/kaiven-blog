...
这篇的三个读数都属于同一份 3 MiB 的数据：在老家每步 9.9 ns，被迫搬家那一遍 69 ns，住定之后又回到 10.2 ns。服务器的内存落户同理，只是税更重、且按月征收。下一篇钻进内存条内部：第二篇的跨步实验里，行越稀疏每行越贵，那个坡是内存自己的顺序偏好。Rank、Bank、电容矩阵，CL、tRCD、tRP 各在什么时候付钱，一篇讲清。
