...
全表扫描 1000 万行（6.4 万页）刚走完，`INNODB_BUFFER_PAGE_LRU` 里 8 个 users 热页一个都没被冲走：5 个在队首 8187-8191，3 个在 old 区中段等着被重访接应。谁拦的？

第一篇结尾埋了一句话没还：回表 40 万页读 vs 覆盖 233 页读、耗时却只差 4 倍，因为「buffer pool 兜住了随机读，表大于内存时才会现出真实代价」。这一篇就来把 buffer pool 本身拆开：**一页从磁盘进内存住在哪、凭什么留下、又被谁赶走**。它是页、区、段、表空间这套磁盘四层结构之外的内存层，也是前六篇每个「页读」计数器背后的那只手。
...
本轮新读入的五个页全落在 545-549，正是 old 子链头部（当时 old 区从 543 延伸到 546 一带）。「叶子在 old、根在 young」不是巧合，但根和中间页站在 young 区的原因**与被晋升无关**（预热期根本不晋升，见下节），是边界滑动：新页不断从中点塞进 old 头部，old 区超长越过容差，边界指针就向队首让一步、把最靠上的几个 old 页顺手翻成 young，**没有再访问也能「集体转正」**。（带 \* 的三个页就是这么翻的。）这也是拿 `IS_OLD` 做观测时的一条纪律：NO 未必代表晋升，YES 也未必代表冷。边界 ±20 页以内的身份牌，看边界滑动，别看页本身。

### 边界是一条会滑动的带

`buf_LRU_old_adjust_len` 里有个 `BUF_LRU_OLD_TOLERANCE`：边界指针并不每插一页就挪一步，**攒够容差才动**。源码是个双向循环：
...
池子从没满过、一个页都没淘汰过（`freed_page_clock == 0`）时，**所有升 young 请求直接拒绝，连计数器都不走**。我在这上面踩了个大坑：冷启动后点查几个页、隔 2 秒再点一次，`made young` 一点不动。原因与时间窗无关：**预热期根本不挪页**。InnoDB 的逻辑是：池子还有的是空位，谁也别折腾。要看到升 young，先得让淘汰发生（把池子灌满）。这个冷知识网上几乎没人提，因为文档只写「young/old 子链」，不写预热豁免。
...
而这一切发生在 65460 页的扫描洪水之后。百万行扫描冲进来又流出去，**8 个热页一个都没被逐出**：扫描结束的快照里，5 个已在 young 区的页待在队首附近（8187-8191），3 个 old 页守在 old 区中段（3038-3040）等这轮重访接应。扫描自己的页呢？`Old database pages = 3043`，洪水全被圈在 old 子链里，淘汰从队尾（old 的末端）开始，牺牲的永远是扫描自己的页。
...
热身完成、中点插入和时间窗都验完了，现在兑现那句承诺。搭一个「表大于内存」的场景：big_k 表 1000 万行，聚簇树 1044MiB + 二级索引 idx_k 193MiB，池子 128MiB（约 5 万页 ≈ 表的 7%）。idx_k 的键是 id 乘大素数取模的双射，**k 与物理页序完全打散，每 31 个相邻的 k 才落同一个索引叶**，回表时的聚簇页也是纯随机访问。
...
缓冲池不只装数据页，还住着两种辅助结构，8.4 正好都改了默认值，各自实测一遍。
...
扫描洪水冲不掉热页，是三道闸的合力：6.5 万页的扫描只放行 0 个升 young、拒绝 127589 次；热页隔 3 秒重访升 young 站上队首，百万页洪水从 old 尾逐出。把时间窗拧到 0，同一个扫描立刻变成 56078 页升 young 的洗劫。LRU 会不会被扫描冲垮，取决于这个默认值为 1000 的旋钮。

表大于内存时，回表的债终于现形：覆盖 0.72 ms / 0 物理读，回表 57 ms / 377 物理读，80 倍时差、每行 1.26 页物理读。第一篇的「4 倍会变 40 倍」兑现为实测 80 倍。且回表拉进来的 300 多个聚簇页里 350 个落在 old 区拿不到晋升资格：**随机回表是缓存杀手，杀伤力连「进池」都到不了，更别提「驻留」**。覆盖索引的成本，从此该用物理读来算。

8.4 关掉了两种辅助结构的默认开关。change buffer：none vs all 实测插入快 20 倍、物理读省 90%（20472 → ~1800 总账），但合并风暴与 SSD 时代收窄的受益面让默认翻成 none（9.x 已正式弃用）。AHI：同一模式访问上万次才攒够建哈希的分数，实测开关无差异，默认翻成 OFF 是把 latch 争用的历史包袱放下。**缓冲池的默认配置哲学变了：从「宁滥勿缺」到「证据不足先不开」。**

下一篇接着写路径讲：change buffer 的「推迟落盘」背后，是「改页前必须先记日志」的另一半。binlog 与 redo 的两阶段提交、组提交与崩溃裁决，脏页刷盘引向提交那个最贵的瞬间。
