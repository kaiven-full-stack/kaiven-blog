...
description: 256MiB 的 write 用 0.12 秒就返回了——盘根本没碰过。本文拆开 write 之后的漫长旅程：page cache 怎么让读写都变成内存操作、脏页在账本上怎么记、flusher 后台线程按什么水位和时钟干活、fsync 到底承诺了什么；实测三种 fsync 姿势的吞吐差（2114 MB/s 的纯 write、605 MB/s 的段段 fsync），把 Redis appendfsync 三档策略的丢失窗口落到内核时间线上。实验在 Linux 7.2.3 与 btrfs 上复核，源码对照 vanilla v7.2。
pubDate: 2026-09-10
category: kernel
...
```text
13:57:35  write 256MiB，用时 0.121 秒（2114 MB/s）。
13:57:35  Dirty 涨到 25MB，盘还没动。
13:57:40  flusher 后台线程醒来，脏页开始流走。
13:58:36  另一轮：段段 fsync，同样的 256MiB，605 MB/s。
```

第一行和第四行之间隔着三倍的速度差，中间只差一件事：要不要等盘。

> 读完这一篇，你应该能回答三个问题：**write() 返回时数据到底在哪？内核在什么条件下才真正写盘？fsync 的承诺精确到哪一层、不含哪一层？**（前置概念见[《第 0 篇》](/posts/kernel-primer/)：页、/proc 账本。）

Redis 系列拆 AOF 时留过一个尾巴：`appendfsync everysec` 的丢失窗口约一秒，但这「一秒」在内核里是什么、由谁计时、为什么恰好是一秒，那篇只说到「交给内核的脏页机制」。VMA 篇收尾时也埋了钩子。这一篇还账：从 `write()` 返回的那一刻开始，跟着一页数据走完它到盘片的全程。

实验环境：Linux 7.2.3，AMD Zen 2，NVMe SSD，根分区 btrfs——文件系统不是背景板，本篇的多个实验结果都带着 btrfs 的指纹，遇到时会单独说明。源码对照 vanilla v7.2 tag。观测主账本是 `/proc/meminfo` 的 `Dirty`（已改未写的页）和 `Writeback`（正在写盘的页）两列。
...
**规则一：时钟。** `vm.dirty_writeback_centisecs`（本机 1500）——每 15 秒醒一次扫账。
...
两个读数值得停一秒。**其一，256MiB 的 write 只让 Dirty 涨了 25MB**——这不是账本坏了，是 btrfs 的指纹：文件系统有自己的写缓存与合并层，数据先进入 btrfs 的内部结构（这部分常常直接进入它自己的落盘路径），最后一批才以脏页形式留在 page cache。换成 ext4/xfs，Dirty 会涨得更接近写入量——**换文件系统，write 的可见行为就变**，这是「版本是结论的一部分」在文件系统维度的扩展。
...
fsync 的承诺精确地说：**把这个文件的内容和必要的元数据推进到存储设备，等设备确认**。它返回后，进程崩了数据还在；但注意两个不含——不含同机断电以外的一切（设备自身缓存另说），也不含全局 Dirty 清零（只管自己文件的页）。实验 D 拍到了这个细节：
...
fsync 返回瞬间，自己文件的脏页清账（落到设备），系统背景脏页原样躺着。**Dirty 是全局账，fsync 是单文件动作**——读监控时把这俩混在一起，会得出「fsync 没用」的假结论。
...
三个数字把三档策略的代价摊开：**always 的 605 MB/s 不是盘慢，是 64 次往返**——每次 fsync 都要等设备确认，把流式写切成段段确认；纯 write 的 2114 MB/s 是内存速度的假象；末尾一次 fsync 拿到最高的「确认后吞吐」，代价是整个写入窗口的数据都悬在内存里。Redis 选 everysec 是中间价：一秒的窗口，最多丢一秒，吞吐几乎不受影响。
...
**write() 是一次内存拷贝。** 数据进 page cache、页标脏、立刻返回；盘在不在、快不快，这一刻都不重要。读写共享这本缓存，`Cached` 那 5GB 是系统免费的加速器。

**回写由时钟、水位和写入者自己三重驱动。** flusher 每 15 秒醒或脏页过 64MiB 被唤醒，30 秒最老脏页优先；脏页逼近 256MiB 硬顶时，**写入者被按住睡眠**——「write 快」的自由区以水位为界，内核绝不让脏页灌满内存。

**fsync 只承诺自己的文件。** 返回 = 该文件内容与必要元数据已被设备确认；不承诺全局 Dirty 清零、不承诺别的进程的数据。丢失窗口的本质是「从未见过盘的 write 集合」，进程崩溃不扩大它，断电才收割它。

**三档策略是三张价目表。** 纯 write 内存价、段段 fsync 往返价、批量一次 fsync 窗口价。Redis everysec 的「最多丢一秒」翻译成内核语言：一秒窗口内的 write 全部悬在 page cache 与文件系统缓冲里。文件系统不同，窗口内的可见行为就不同——btrfs 的中间层让 256MiB 只露 25MB 的脏。

---

```text
13:57:35  write 返回，数据躺在缓存里，像已经寄出的明信片。
13:57:40  flusher 醒来，一批批把明信片投进邮筒。
13:58:36  段段 fsync 的那位，每写一段就在邮局等到盖戳。
```

page cache 是这个系列最后一块大账本：页表管虚拟侧、伙伴系统管物理侧、slab 管内核自己的小对象、page cache 管文件的页。到这里，「分配」的故事全部讲完。剩下最后一个角色，站在所有账本的尽头：内存见底时，内核如何决定杀谁——OOM killer，以及那道最后的保险丝。
