...
description: 200 字节内容为什么会成为 233 字节对象，又落进 240 字节 block？本文从 PyObject 与 PyVarObject 的公共头部出发，拆开引用计数、类型指针、可变长度、GC 前置元数据、内联数据与间接缓冲区，辨清 sys.getsizeof、对象浅尺寸和分配器实际承载之间的边界。实验在 CPython 3.14.7 上复核，并对照 CPython 3.12.13 与 3.16.0a0 开发源码。
pubDate: 2026-09-08
category: cpython
...
上一篇追的是第三个数字：233 字节的小请求进入默认 pymalloc 后，被归入 240 字节 size class，住进一只固定规格的 block。这一篇倒回申请发生以前，看看前两个数字为什么不同，以及那 233 字节里面究竟放了什么。
...
本文实验以 **CPython 3.14.7、x86_64 Linux、64 位、默认 GIL、非 debug、默认 pymalloc 构建**为主，并与 CPython 3.12.13 做对照。源码结构同时核对本地一份标记为 **CPython 3.16.0a0** 的开发快照；该目录没有 Git 元数据，无法绑定到具体提交。文中的对象字段、典型字节数和分配路径属于当前 CPython 版本与构建事实，不是 Python 语言对其他实现许下的内存布局承诺。
...
`ob_refcnt` 所在区域维护对象生命账本。上一篇已经追过 `Py_INCREF()`、`Py_DECREF()`、immortal object 与引用归零；这一篇只关心它占据公共前缀的一部分。
...
`__basicsize__` 对应类型的 `tp_basicsize`，描述固定主体；`__itemsize__` 对应 `tp_itemsize`，描述每个尾随变长项增加多少字节。但它们也不是对象完整账单：GC 前置区、managed preheader、inline values、独立缓冲区和 allocator 取整都可能在公式之外。

因此更准确的说法不是“所有 Python 对象头都是 16 字节”，而是：

> 在本文典型 64 位默认 GIL 构建中，普通 `PyObject` 公共前缀是 16 字节；具体对象还要叠加类型自己的字段、可变内容与可能位于对象地址之外的元数据。

等到自由线程构建，这张公共登记卡本身也会变样，本文后面再划出边界。

## `ob_type` 不只写着名字，还规定房型
...
而且类型不只规定布局。`ob_type` 还通向数值运算、属性查找、调用、迭代与析构等类型槽。同一个 `+` 为什么会落到整数加法、字符串拼接或用户自定义方法，入口就藏在这枚指针之后。那是对象布局通向下一篇执行机制的桥，此处先停在尺寸上。
...
`0` 仍至少为一个 digit 预留逻辑尺寸。越过 30 bit 边界以后，新的 digit 才加入账单。这里的 28、32、36 不是 allocator block 规格，而是 `int.__sizeof__()` 按当前内部表示报告的浅层尺寸。
...
这三笔账还没算 GC 头，因为 bytes 不是循环 GC 管理的类型；也没有独立 backing array，因为数据已经内联。正因为例子足够干净，它适合作为对象布局的第一把尺。
...
因此“Python 字符串一个字符占几字节”缺少至少两项条件：哪一种内部表示，以及在称字符数据还是整个对象。字符串驻留、子类化、UTF-8 cache 和版本变化还会继续改变账单。
...
额外的 32 字节来自当前普通实例类型的 preheader 账单。`sys.getsizeof()` 信任类型给出的 123，并没有去 allocator 验证“真实 block 是否正好这么大”。
...
**对象头是运行时协议，不是业务内容。** 在本文默认 64 位 GIL 构建中，`PyObject` 的引用计数状态和类型指针占 16 字节；具体类型还会继续增加自己的字段。

**`ob_type` 决定一块内存应怎样解释。** `tp_basicsize`、`tp_itemsize` 与类型专用分配逻辑共同算出结构尺寸，同一枚类型指针还通向操作和析构槽。

**可变长不等于统一使用 `ob_size`。** tuple、list、bytes 使用 `PyVarObject` 协议；现代 int 把 digit 数和符号编码在 `lv_tag` 中，并使用专门公式。

**数据可以内联，也可以间接拥有。** bytes 把内容放在对象尾部，tuple 尾随元素指针，list 则拥有独立的引用数组；空间位置与记账归属不是同一件事。

**`sys.getsizeof()` 是不完整的浅层协议，不是内存真相仪。** 它先调用类型的 `__sizeof__()`，再加入当前类型 preheader；不会自动递归引用图，也不保证覆盖 inline values，更不会读取 allocator block 或 RSS。

**共享对象不能重复记账。** 一千个引用槽可以全部指向同一只对象；深度统计必须先定义边界，再按身份去重并处理环与共享状态。

**`__slots__` 省掉的是通用属性存储，不是对象头，也不是全部属性成本。** 普通 slot 仍占一个指针字段，继承、weakref 与类布局都会改变结果。

**GC preheader 与 managed preheader 不是同一块。** 默认 GIL 构建中的 `PyGC_Head` 管理 GC 链表；managed dict/weakref 另有两个前置指针槽。

**类型 freelist、pymalloc 与 RSS 位于不同层。** 旧对象壳可能先被类型缓存，block 可能再被分配器复用，进程页面则是更下一层的账。
...
```text
200 字节，是住客带来的行李。
233 字节，是行李连同登记卡与房内设施占去的空间。
240 字节，是前台按照固定房型交出的钥匙。
```

三份账都准确，只是没有在称同一件东西。

上一篇站在分配器前台，看 233 字节怎样住进 240 字节 block；这一篇走进房间，翻开公共对象头、可变长度、尾随数据与寄存在别处的引用数组。等灯亮起来，解释器还要沿 `ob_type` 找到这只对象会做什么：同一个加号该走整数运算还是字符串拼接，属性读取又该翻哪一本登记册。

对象有多重，到这里已经有了分层答案。下一篇便沿着 `ob_type` 继续向前，看看一块已经排好字节的内存，怎样获得属于自己的行为。
