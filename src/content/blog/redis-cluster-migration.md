...
description: 同一个槽迁移期间，旧键仍在源节点，新键却能经 ASK 先落到目标节点。本文沿一次真实的槽迁移，拆开 16384 个槽、MOVED、ASKING、MIGRATE、TRYAGAIN 与 configEpoch 各自的职责。实验在六只 Redis 7.4.11 容器组成的隔离集群里完成。
pubDate: 2026-09-05
category: redis
tags: [Redis, 数据库]
---

槽 5061 从 m1 迁往 m2 的过程中，客户端经历了一次容易看错的切换：迁移开始时向 m1 写一个不存在的新键 `{bar}new`，m1 回答 `-ASK 5061 172.28.0.12:6379`，客户端向 m2 发送 `ASKING` 再执行 SET，成功——可槽的正式 owner 仍是 m1。直到最后一个键搬完、槽位改属 m2，m1 才对后来者改口 `-MOVED`。客户端在门牌正式改掉以前，已经临时换过一次门。

`ASK` 与 `MOVED` 都会把客户端指向另一个节点，却不能按同一种方式处理。`ASK` 只说“这一条命令先去那里”，客户端不得更新长期槽位表；`MOVED` 才说“槽已经换了主人”，客户端应刷新地址簿。前者发生在键还分散于两端的迁移窗口，后者发生在归属正式变更以后。

实验使用六只受限的 Redis 7.4.11 容器组成三主三副本 Cluster，运行在 internal Docker 网络，不映射宿主机端口；键数量很少，所有容器、匿名卷和网络均已清理。故障转移与传播时间只描述本次机器。

上一篇特意说明，Sentinel 没有 `MOVED`、`ASK`、`ASKING` 和槽位。Sentinel 负责告诉客户端当前主库是谁；Cluster 则把“你敲错门了”写进每次请求的协议。本文不从一致性哈希环讲起，而是沿一次真实的槽迁移，逐项看数据和归属怎样分开移动。
...
最终每个槽有且只有一个正式 owner。客户端不必连接某个中央代理，可以连接任意节点；若敲错门，节点通过重定向告诉它下一步去哪里。Cluster 没有统一入口，它依靠所有节点共同维护一张槽位归属表。
...
为什么不是完整的 65536 个槽？槽位 bitmap 会进入节点心跳。16384 位刚好是 2KB，65536 位会变成 8KB；再结合 Cluster 预期支持的主节点规模，16384 是传播成本与分片粒度的工程折中，与“16 位除以四”无关。
...
`MOVED` 不代表 Cluster 出了故障，它是客户端路由表陈旧时的正常纠正协议。
...
`MOVED` 是正式改址通知，`ASK` 只对紧随其后的那一条命令有效。
...
这是一条重要边界：Cluster 全局传播正式槽位 owner 与 `configEpoch`，不会替迁移工具保存每一步迁移状态。控制端若崩溃，需要从源、目标两端重新检查并恢复。**迁移状态属于两端节点，正式归属才属于整个集群。**
...
半途停下并不可怕，危险的是忘掉临时路由为什么存在、把标记先清了。
...
实验在目标、源和第三方节点上依次执行 `SETSLOT NODE`，第三方立即改为 `MOVED` 到 m2；由于主动通知，未观察到可见的 gossip 延迟。这只能证明工具主动更新有效，不能证明纯 gossip 在任何环境下零延迟。归属表最终一致，重定向负责跨过传播窗口。
...
正常复制中的键完整存在。迁移中途故障的路由窟窿则来自实现边界：本地迁移标记没有被复制。问题不在副本选举，而在迁移状态必须由外部控制器恢复。**故障转移继承正式归属，不继承半途的迁移状态。**
...
`IMPORTING` 必须先于 `MIGRATING`，否则 ASK 没有落点；`ASK` 不能更新槽表，否则两端乒乓；`MIGRATE` 超时可能留下重复键，控制器要核对两端；半途 `STABLE` 会切断临时路由；目标先接槽，是为了避免无 owner 窗口；第三方节点看不见迁移过程，只认最终 owner 与 epoch。

`MOVED` 与 `ASK` 给出的地址可能完全相同，客户端行为却必须不同：前者宣告归属已变，后者只在迁移窗口内放行一条命令。

Redis Cluster 不会移动正在敲门的客户端。它只改写归属，然后等下一次请求带着旧地址来找它，再用重定向把客户端引到对的地方。
