---
title: 同一个加号，为什么知道该找谁：CPython 的类型槽与操作分派
description: 一枚 + 为什么能找到整数加法、字符串拼接与用户定义的 __add__？本文沿 ob_type 进入 PyTypeObject，从 BINARY_OP、PyNumber_Add 与 nb_add 追到反向运算、严格子类优先、NotImplemented、序列拼接后备和类型槽更新，辨清 Python 数据模型与 CPython 当前实现各自保证到哪里。实验在 CPython 3.14.7 上复核，并对照 CPython 3.12.13 与 3.16.0a0 开发源码。
...
上一篇沿对象头里的 `ob_type` 走到 `PyTypeObject`，先读取 `tp_basicsize` 与 `tp_itemsize`，回答一块内存有多大、该怎样解释。可类型对象保存的不只是房型。表达式 `left + right` 到来时，解释器还要沿同一条指针寻找操作入口，再判断左右哪一方愿意处理这对对象。

这一篇不再称对象的重量，而是追一枚加号怎样找到答题人。

本文实验以 **CPython 3.14.7、x86_64 Linux、64 位、默认 GIL、非 debug 构建**为主，并与 CPython 3.12.13 做对照。源码结构同时核对本地一份标记为 **CPython 3.16.0a0** 的开发快照；该目录没有 Git 元数据，无法绑定到具体提交。`__add__`、`__radd__`、`NotImplemented` 与严格子类的反向优先属于 Python 数据模型；`BINARY_OP`、`binary_op1()`、槽包装函数和专用指令名称则是当前 CPython 实现事实。
...
`tp_dealloc` 在引用计数篇里已经出现过：对象死亡时，类型决定怎样清理自己。`nb_add` 则站在另一端：对象还活着，一枚 `+` 到来时，类型决定自己如何参与加法协议。
...
`dis` 输出同样不是 Python 语言规范。不同版本可以改变指令名称、合并方式和缓存呈现；本文只用它定位 CPython 3.14.7 中从表达式进入运行时的那扇门。
...
但对 Python 定义的类，还要再加一层：左右类的 `nb_add` 经常都指向同一只通用 `slot_nb_add` wrapper。顶层 C 槽只调用一次，不意味着 Python 层永远不会调用 `__radd__`；wrapper 内部还会完成正向与反向方法的二次仲裁。
...
```text
return NotImplemented      这一位答题人转交
raise NotImplementedError  当前执行直接失败
最终 TypeError             整套协议无人接题
```
...
绕过实例并不是只为加法定制的古怪例外，而是隐式特殊方法查找的一般设计。它让 `len(value)`、`value + other` 等协议直接连接类型槽，避免单个实例任意影射核心操作。
...
还有一个限制：`int` 这类 immutable static type 通常不允许直接执行 `int.__add__ = ...`。这不是槽更新机制只支持用户类，而是类型本身带有不可修改标志。Python 定义的可变子类仍可以安装自己的特殊方法与槽 wrapper。
...
**`ob_type` 不只解释对象的字节，也连接对象的行为。** 类型对象既给出尺寸与析构协议，也提供数值、序列等操作入口。

**一枚 `+` 先进入抽象操作协议，不等于普通调用 `left.__add__(right)`。** 实例同名属性、严格子类优先、反向实现与序列后备都会让这条等式失真。

**一个 `nb_add` 槽承载整套加法协商。** 对 Python 类，`__add__` 与 `__radd__` 常通过同一只 `slot_nb_add` wrapper 接入，并非两只互不相干的 C 槽。

**`NotImplemented` 是转交信号，不是异常。** `NotImplementedError` 会立即传播；两侧都拒绝且无其他后备时，外层才抛 `TypeError`。

**右侧反向实现既可能是后备，也可能因严格子类关系获得优先权。** 但子类必须提供不同反向语义，只继承同一实现并不会自动插队。

**同类型不会重复询问同一份反向协议。** 顶层槽和共享 wrapper 都会避免把同一能力换个方向再调用一次。

**字符串与列表拼接体现的是左侧 `sq_concat` 后备。** 数字槽先协商，随后才看左序列槽；没有一套对称的右侧序列拼接仲裁。

**特殊方法从类型出发，实例同名属性不能替换隐式操作。** 显式 `obj.__add__()` 与 `obj + other` 走的是不同查找入口。

**类型槽可以动态更新。** 修改类上的特殊方法会重算槽并影响已有实例；删除后是否清空，还取决于 MRO 中是否存在继承实现。

**特化可以缩短路径，不能改写协议语义。** 专用指令的 guard 不成立时，解释器必须回到等价的通用分派。
...
整数把答卷交给 nb_add。
字符串从序列后门接过加号。
左边写下 NotImplemented，右边才有机会续答。
更具体的子类有时会先拿到笔。
```

同一个符号，没有预先认定某一种对象。解释器先从 `ob_type` 找到类型，再沿操作槽安排答题顺序；每一次返回值都在告诉分派层“已经处理”“继续转交”或“发生异常”。

这一篇回答了语义上该找谁。可若一段循环里连续做一百万次 exact int 加法，每次重新翻完同一套登记册便显得昂贵。下一篇继续留在这枚加号上，不再问谁有资格回答，而问答案长期不变以后，CPython 怎样记住它：何时尝试特化，inline cache 保存什么，专用指令凭什么走短路，类型忽然改变时又怎样回到通用分派。
