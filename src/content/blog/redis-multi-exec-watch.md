...
description: 入队时参数写错，整个事务当场弃单；执行时类型不对，只有那一条报错，后面的照常跑。本文把 Redis 事务的两道检查门、没有回滚的设计理由、WATCH 的乐观锁协议与过期键特例逐一拆开，并回答官方那个著名建议：为什么 Lua 脚本更快更简单。实验与源码核对都在 7.4.11 上完成。
pubDate: 2026-09-10
category: redis
tags: [Redis, 数据库]
---
...
Redis 的回答是：**它从来没承诺过你熟悉的那种事务**。MULTI/EXEC 提供的是「打包」和「隔离」：队列里的命令连续执行，中间不插入其他客户端的操作；它不提供「要么全成功要么全失败」的原子提交，更没有 ROLLBACK。官方文档对此毫不讳言：不支持回滚是因为错误应当由开发进程解决，而不是留给运行时兜底。语法错误在入队时就被拦下，类型错误是程序 bug，靠回滚掩盖只会让 bug 藏得更深。
...
事件循环篇讲过那个核心事实：**键空间命令由主线程串行执行**。EXEC 收到后，`execCommand` 把队列里的命令逐条送进同一个 `call()`，中间不回到事件循环：对这个客户端来说是原子批处理，对其他客户端来说，这批命令和一条命令没有区别。隔离由单线程执行模型免费提供，事务机制自己没有做什么。

一个容易被忽略的细节是传播。EXEC 期间每条命令的 AOF/副本传播被延迟到整个执行单元结束，由 `propagatePendingCommands()` 统一发送，所以**副本看到的事务也是完整的一批**，不会出现主库 EXEC 了一半、副本只重放半批的窗口。AOF 篇讲过的命令流水，在这里以「批」为单位传播。

还有 ACL 的一层：入队时检查过权限，EXEC 时会**再查一遍**，因为管理员可能在你排队和执行之间改了权限。这类错误同样只作废当前条目并回复 NOPERM，不影响批的其余部分。

## WATCH：用弃单替代锁

到目前为止，事务还只是「打包」；真正的并发控制由 WATCH 补上，而它的思路是乐观锁：
...
`watchForKey` 把键登记进数据库的 `watched_keys` 字典。此后**任何客户端**修改这些键（SET、DEL、过期删除、甚至另一个事务），`touchWatchedKey` 都会顺着字典找到所有观察者，打上 `CLIENT_DIRTY_CAS`。EXEC 开头验标记：脏了就返回空数组（这是 nil 多批量回复，不算错误），整单作废；干净才执行。
...
MULTI/EXEC 是打包与隔离，不是原子提交：队列里的命令连续执行、不插队，这由单线程模型免费提供；但没有回滚，执行期错误只报错那一条，其余照常。两道门的分工要记牢：入队期错误（语法、参数数量）触发 EXECABORT 整单弃掉，执行期错误（类型、权限变更）逐条报告、批不散，写代码时应该只依赖第一道门。WATCH 是乐观锁，用弃单替代锁：任何客户端的修改都会弄脏观察者，EXEC 返回 nil，客户端重读重算重试；WATCH 时已过期的键视同不存在，其后的物理删除不算修改。Lua 脚本是「读了再写」的正解：事务里不能读是 MULTI 一切别扭的根源，脚本把读算写搬进服务端单命令边界内，WATCH 和重试都可以省掉，代价是脚本要短。最后，别用 MULTI 提吞吐：流水线才是批量工具，MULTI 的队列拷贝和执行调度是净开销，买的是语义不是速度。

对「事务」这个词的预期越像关系型数据库，这套协议就越显得简陋；把它看成「一批命令的免插队通道 + 一枚版本探针」，每个设计选择就都对上了。

---

系列至此第十二篇。命令串行执行是隔离的来源，见《一个命令没走完，所有人都在门外》；EXEC 期间以批为单位传播给副本与 AOF，见《回复你 OK 的那一刻，还不算写进历史》；WATCH 过期键特例里「逻辑不存在 vs 物理删除」的分离，见《过期的键，不会准时消失》。
