...
description: Zig 没有 string，没有 char，字符串是 []const u8 加上一份约定：格式串在编译期查过，拼接要自带分配器，中文按 UTF-8 字节计数。文中代码在 0.16.0 上逐一验证。
pubDate: 2026-09-09
category: zig
tags: [Zig, 编程语言]
---
...
这篇就把这串问题一次答完。先说结论：Zig 不是缺了字符串，是不肯假装字符串是一种东西。字符串在这里是一串字节，加一份写清楚的约定。代码照例全部在 Zig 0.16.0 上验证过。
...
指针那篇拆过这个形状：只读指针，指向长度为 6、以 0 结尾的字节数组。六个字节装下两个汉字（UTF-8 里一个汉字三个字节），外加一个哨兵 0。它携带的证据比 `[]const u8` 多，所以能按需放下一部分，隐式变成普通切片。

但这份偏爱到此为止。一旦变成 `[]const u8`，类型系统对它的态度就和对待任何切片一样：这里有一段内存，从哪开始，多长，只读。至于这些字节是不是合法的 UTF-8、是不是文字、该按字节还是按字符理解——类型不知道，也不假装知道。

没有 string 类型不是省了一个关键字，是省掉了一整套假装。别的语言里 `string` 是个容器，带容量、带编码、带方法；Zig 里它只是别人内存上的一张借条。借条的规矩，切片那篇已经写透了；这篇要补的是另一边：既然语言不认「字符串」这个概念，那些字符串该有的日常手艺——打印、比较、拼接、数长度、遍历字符——各自去哪儿认领。
...
而这一整套检查发生在编译期，是因为格式串本身就是一个 comptime 参数。试着把格式串当普通值传，就会撞上那堵墙：
...
格式串必须在编译期就知道，所以占位符与实参的匹配、说明符与类型的匹配，全是编译器替你查的。等价的检查在别的语言里是运行时异常，或者干脆没有检查。这也是 comptime 那篇的老话：不是加了一道门禁，是把检查从运行时搬进了编译器。

代价也直说：格式串没法运行时拼出来。日志库想允许用户配置输出格式，就得自己走解析那条路，标准库的 `{s}` 体系帮不上忙。这份灵活性被换来换成了确定性。
...
顺带一个实验结果：两个字面量 `"ink"` 与 `"ink"`，`s1.ptr == s2.ptr` 是真的——编译器把相同内容的字面量合并到了同一地址。但这是这次编译的优化行为，不是承诺；今天靠它，明天换 `-O` 级别就可能是另一回事。地址相等能推出内容相等，反过来不成立。比较内容，用 `eql`。
...
再往下一层还有个容易漏看的细节：`'A'` 也不是 `char`——Zig 没有 `char` 类型。它是 `comptime_int`，值 65：
...
`++` 要求两边都是编译期已知的数组，拼出来的还是那个 `*const [N:0]u8`，静态存储，零运行时成本。它本质是类型层面的操作：长度相加在编译期就算好了。
...
分配器那篇的老话在这条沟里又响了一遍：分配是调用方的决策。别的语言里 `"a" + b + "c"` 背后是运行时悄悄扩容的缓冲区；Zig 把那块缓冲区摆到了签名里。写起来确实多两行，换来的是拼接的成本和归属都在明面上——谁分配的、谁负责还、什么时候还，一眼可查。
...
`Writer.Allocating` 是个自己带 ArrayList 的写入器，`print` 按需扩容，最后 `toOwnedSlice` 把积累的字节正式移交。它就是其他语言里 StringBuilder 的位置——只是这里它不叫 Builder，叫「一个会分配的 Writer」，而且谁给它分配、何时结清，都在签名和 defer 里写着。
...
日常操作里藏着一条贯穿性的分界：哪些函数借走你的字符串，哪些替你誊一份。分界线几乎总跟着 allocator 走。

借的一侧——不分配、不修改，结果只是输入的另一个视角：
...
誊的一侧——要分配，要报 `OutOfMemory`，要有人负责释放：
...
分辨的方法切片那篇给过：看函数接不接受 allocator。接受，多半意味着「我要分配新的」；不接受，多半意味着「我只借用」。`trim` 与 `concat` 是这条规则最干净的一对例子。借用者便宜，但寿命受制于原件；誊写者自立门户，但要把分配和释放写进你的账本。
...
一个以「小内核 + 显式依赖」为哲学的标准库，不可能把这张表焊进来。于是线画在这里：字节操作、ASCII、UTF-8 编解码验证，标准库管；字素切分、大小写映射、本地化排序，专门的库管。这在中文语境里损失不大——中文几乎用不到大小写——但排序是实打实的：`std.mem.order` 按字节比较，「听」排在所有 ASCII 之后，按拼音排序列表这种需求，标准库帮不上。

同样的取舍在时间格式化、复数规则、纸币符号这些地方反复出现。Zig 的答案始终是：语言和标准库不替文化做决定。

## 代价，认真地

**没有默认，就没有意外。** `{s}` 要写、比较要点名、拼接带 allocator，每一处都比「字符串魔法」多几个字。换来的是打印格式、比较语义、内存归属全都显式可见，代码评审时无一处靠猜。这笔账在小事上亏，在长命代码上是赚的。
...
**format 的编译期检查很严，也框死了运行时格式。** 占位符、类型、说明符全在编译期对账，写错立刻红。但格式串也因此不能运行时构造——需要用户可配置模板的系统，得自己实现一层解析。安全性与灵活性在此处明码标价，各取一半。

**小工具丰富，文化问题出局。** `std.mem` 里 trim、split、indexOf、startsWith 一应俱全，零分配的路径铺得很顺；过了 Unicode 码点这条线，标准库就退场了。字素、大小写、本地化排序，都要引入外部依赖，而且体量不小。中文用户对「大小写」无感，对「排序」有感——这块成本在中文项目里反而是最常被低估的。

---

回到开头那串问题。

为什么打印要 `{s}`——因为一串字节有多种解释，解释权在你。为什么不能用 `==`——因为指针和内容都叫「相等」，Zig 不替你选。为什么拼接要 allocator——因为新字符串要住进新内存，内存从哪来是调用方的事。为什么 `"听雨".len` 是 6——因为它本来就是六个字节，字才是两个。

前文谈切片，看借据与期限；谈指针，看证据如何随类型增减；谈分配器，看内存的来路如何写进签名。这一篇把这些线拧在「字符串」这根日常的轴上：所谓没有 string 类型，其实是把别的语言焊在 string 里的一堆默认决定——编码、比较、扩容、本地化——逐个拆了出来，摆回各自的明处。

一串字节躺在内存里，本身不是文字。是人来认领它：认领它的编码，认领它的边界，认领它的归属。Zig 只是不肯替你认领。

夜雨敲窗，案头一行 `[]const u8`。灯下细看，哪几个字节是「听」，哪几个是「雨」，纸上是不会写的——认字的人自己知道。
