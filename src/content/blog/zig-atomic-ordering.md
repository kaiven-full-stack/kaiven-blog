...
description: 生产者先写数据再挂标志，消费者先见到标志再读数据，两边各自的观察都对，合在一起却未必对得上。这篇分清原子性、可见性、release/acquire 与全序各自的管辖范围。实验环境为 Zig 0.16.0、x86_64 Linux。
pubDate: 2026-09-05
category: zig
tags: [Zig, 编程语言]
---
...
两份记录都是真的。生产者那边，数据在前、标志在后；消费者那边，标志已经到了，数据却未必跟着到。单线程时一份执行顺序就够说明先后；多出一个线程，就多出一份观察。两边各自保持顺序，不代表所有核心看见同一条时间线。

上一篇讲数据怎么跨过 Zig 与 C 的边界，这一篇讲数据怎么跨过线程边界。字节能抵达只是一半，先后关系要另外建立。

Zig 0.16.0 已提供完整的原子内建与 `std.atomic.Value`，但语言参考尚未写出一套完整、正式的 Zig 内存模型。本文涉及 happens-before、release sequence 和全序的解释，依据当前编译器对 LLVM 原子语义的映射与标准库用法；API 签名以 Zig 0.16.0 语言参考为准，ordering 枚举取自 `std.builtin`，合法组合以编译器的 comptime 检查为准。三层会明确分开，不借别人的术语替 Zig 写一份尚未完成的规范。

代码与输出均在 Zig 0.16.0、x86_64 Linux 上验证过。
...
问题不是消费者等得不够久，而是两份观察之间没有一条语言承认的先后关系。把变量改成 `volatile` 也不行：Zig 语言参考明说，volatile 与并发、原子操作无关，它主要服务于内存映射 I/O。
...
输出本身不算证明，推理链才是；这次运行只是确认实现没有明显背离推理。
...
在 x86_64 上，release store 与 monotonic store 常常都只是一条普通 `mov`，acquire load 与 monotonic load 也可能生成相同的 `mov`。这不表示 ordering 没有意义：语义仍约束编译器重排；换到 AArch64 等较弱内存序架构，acquire/release 可能需要不同指令。代码若只因 x86 硬件顺序较强而「跑得一直正确」，移植以后才会显出缺口。能依赖的是源码里的 ordering，不是某次编译出来的汇编。
...
`.seq_cst` 也不只是「更强的 acquire/release」。它还要求所有 sequentially consistent 操作进入一条全局单一顺序，解决某些仅靠成对发布无法约束的多方观察问题。

## release/acquire 是成对的关系
...
所以先写清算法需要什么——两两同步，还是所有原子操作进入一条共同顺序——再选恰好足够的 ordering，而不是因为 release/acquire 比 seq_cst「更快」就先用它。
...
`generation` 是原子的，不会让 `name` 和 `count` 自动变得线程安全。每个普通字段都必须由同一份同步协议、锁或线程所有权保护。
...
Zig 没有替所有原子操作默认选择 `.seq_cst`。不设默认、要求明说，在这门语言里已经见过不止一次——分配器如此，错误处理如此；成本和语义不藏进默认行为里。
...
文章中的运行输出都只能说明探针成功执行，不能证明所有 interleaving 都正确。并发代码有一种危险的通过方式：在一台机器上跑很久，从未失败。
...
两份记录第一次对上了。不是两颗核心的时钟忽然一致，也不是所有内存操作从此排成一列——只是 P2 与 C1 之间建立了一条同步边，于是 P1 发生在 C2 之前。happens-before 没有消灭并发，只是给必须有先后的那部分事实定了方向。

剩下的就是选型：计数用 monotonic，交接用 release/acquire，多方协议进 seq_cst 全序。ordering 越弱，允许的执行情况越多，要自己证明的也越多。这条判断标准，比任何「哪个更快」的经验都靠得住。
