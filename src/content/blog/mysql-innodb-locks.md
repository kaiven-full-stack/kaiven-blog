...
description: MySQL 系列第三篇。同一张五行表上，插 id=7 只要 2.4 毫秒、改 id=5 却等了 9.96 秒，因为锁住的未必是行，可能是「行之间的空隙」。本文用 performance_schema.data_locks 把每把锁逐把拆开：等值命中退化成纯记录锁、打空落成纯间隙锁（X 锁竟能多方共存）、范围查询用 next-key 封隙、开上界锁到 supremum 伪记录；全表 UPDATE 的 5009 把锁 = 5000 行 + 9 个叶子页各自的「无穷」；RC 下一把间隙锁都不设。再现场复现三种死锁（AB-BA、共享间隙双双 INSERT、双 S 升级），逐行读 LATEST DETECTED DEADLOCK；最后是 MDL 门链：一个挂着的事务加一个 ALTER，后续所有查询排队等死，KILL 掉 ALTER 的瞬间队伍毫秒级放行。锁的现场全部来自 performance_schema。
pubDate: 2026-09-12
category: mysql
tags: [MySQL, 数据库, 事务]
---

同一张五行表，同一个对手 A（事务里 `SELECT ... FOR UPDATE` 之后挂着），B 的两条语句命运差了 4000 倍：`INSERT INTO t_lock VALUES (7,700)` 2.4 毫秒落地，`UPDATE t_lock SET v=55 WHERE id=5` 等了 9.96 秒才放行。上一篇（[《旧的那行去哪了》](/posts/mysql-innodb-mvcc/)）结尾留了一个现场：`data_locks` 里两把 `X,REC_NOT_GAP`，一把 GRANTED 一把 WAITING。这一篇把那把锁放到显微镜下：**一条 UPDATE 到底锁住了什么？** 答案远不止「行」：锁的对象可以是记录、可以是记录之间的空隙、可以是每个叶子页末尾那条「无穷」伪记录，甚至可以是一张还没动的表。

实验环境不变：docker 里的 MySQL 8.4.11。主角是一张五行表 `t_lock`（主键 id 为 1, 5, 10, 15, 20，主键之间的空隙就是舞台），量具四件：`performance_schema.data_locks`（行锁现场）、`metadata_locks`（MDL 现场，需先开 `wait/lock/metadata/sql/mdl` 探针）、`INNODB_TRX`（事务清单）、`SHOW ENGINE INNODB STATUS`（死锁日志）。多会话时序用脚本交错控制，所有时间戳来自 `NOW(6)`。
...
两把锁，两种粒度。**IX 是表级意向锁**：行上有 X 之前先在表上插一面旗，判断「这张表里有没有行锁」不必翻叶子页，看旗就行（MVCC 篇抓锁现场时它已经在每条读数里出现）。**`X,REC_NOT_GAP` 是纯记录锁**：锁 id=5 这一行本身，不含任何空隙。而开头那组数字就此有了答案：B 插的 7 落在 (5,10) 之间，行锁管不着，2.4 毫秒过；B 改的 5 是行本体，等 A 提交，9.96 秒。
...
接下来是全篇最反直觉的一组读数。C 也来锁同一个不存在的 id=7：
...
**锁的登记对象是首篇的「最大记录」supremum**：每个叶子页末尾那条永远存在、永远比一切大的伪记录。锁住它，等于锁住「比 20 大的所有可能」。B 插 id=25，等待记录里插入意向锁的另一种形态现身：
...
RR 下的幻读靠两半合围：**快照读那一半靠 ReadView**（上一篇的三条可见性规则，新插的行事务号太新，不可见）；**当前读这一半靠间隙锁**（FOR UPDATE / UPDATE / DELETE 封住空隙，新行插不进来）。两半各管各的读法，缺一不可。
...
RC 的当前读只锁**语句开始时已存在的行**，幻读在当前读这一侧完全敞开，换来的是写入方不受封隙之苦。**隔离级别的选择，落到锁记录上就是「X,GAP 这一列的有无」**：RR 与 RC 的区别，上一篇是快照建几次，这一篇是间隙锁设不设。
...
B 的客户端当场收到 `ERROR 1213 (40001): Deadlock found`，A 若无其事通过（第二把 UPDATE 0.4 毫秒到手）。`SHOW ENGINE INNODB STATUS` 的 `LATEST DETECTED DEADLOCK` 一节（节选，逐字来自输出）：
...
三样东西值得细看。**其一，物理记录只有四个字段**：id、DB_TRX_ID、DB_ROLL_PTR、v，两张用户列加两枚隐藏列。第一篇 `N_FIELDS=6` 的「隐藏列占座」，在死锁日志里原样兑现。**其二，hex 直接可解**：`0000000184f1` = 99569，正是 A 的事务号；那 7 字节 ROLL_PTR 按上一篇的解码表是「段 1、页 1543、偏移 619」，死锁日志顺手给版本链留了入口。**其三，牺牲品怎么挑**：官方规则是回滚代价小者死，按行锁数与 undo 条数计价；本例双方账面完全对称（2 行锁、1 条 undo），InnoDB 选了 (2)，后起步的 B。
...
全表 UPDATE churn（5000 行）之后，两处读数同时看：
...
两处完全一致（第 5009 = 5000 + 9，但先看**多出来的 9 把锁落在哪**）：
...
幻读的封印是两半的：RR 里快照读靠 ReadView（看不见新行），当前读靠间隙锁（插不进新行）；RC 拆掉后一半，间隙锁缺席，1.9 毫秒的 INSERT 畅通无阻。隔离级别的选择，在锁记录上就是 X,GAP 那一列的有无。

死锁是等待图里的环，三种最短的成环路径都实测复现了：AB-BA 互换两行；共享间隙 + 双双 INSERT（持有阶段毫无冲突迹象的隐形环）；双 S 升级 X（FOR SHARE 不是免费保险）。牺牲品按回滚代价挑，死锁日志逐把锁、逐条物理记录可读，hex 里连 DB_TRX_ID 和 ROLL_PTR 都看得见。检测器毫秒级拆环，关掉它就只剩超时兜底，默认 50 秒。

5009 把 X 锁 = 5000 行 + 9 个叶子页的 supremum，`data_locks` 与 `trx_rows_locked` 完全一致。锁是逐把登记、逐把可数的。
...
这一篇的锁全住在内存里。断电重启的瞬间，所有 GRANTED 与 WAITING 凭空消失，可未提交的改动还躺在页面上、旧版本还躺在 undo 里：谁来收拾残局？下一篇拆 redo 日志与崩溃恢复，第一篇页尾的 LSN、第二篇 undo 的逆向抄回，在那里合龙。
